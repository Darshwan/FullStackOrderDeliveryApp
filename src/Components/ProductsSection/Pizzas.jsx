          ...
          <NavbarMiniComponent />
           {cartClicked && (
              <div className="w-[300px] border-black z-999 duration-700 shadow-lg fixed top-30 right-0 text-start mb-4 p-4 bg-green-100 text-green-800 rounded-lg"style={{ zIndex: 9999 }}>
                <p > <strong>{cartClicked}</strong> has been added to the cart! <Link to='/cart' className="underline">CheckOut</Link></p> 
              </div>
            )} 
          <section className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3 px-4 md:px-6 py-12">
            {pizzas.map(pizza => (
              <div key={pizza.id} className="bg-card rounded-lg overflow-hidden shadow-lg transition-all hover:shadow-xl hover:translate-y-[-5px] duration-500">
                <div className="relative ">
                  <img
                    src="https://cdn.pixabay.com/photo/2024/04/23/09/32/ai-generated-8714516_1280.jpg"
                    alt={pizza.image}
                    width="300"
                    height="200"
                ...
                <div className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="text-lg font-semibold text-orange-600">{pizza.name}</h3>
                    <span className={`bg-primary border px-3 py-1 rounded-full text-xs font-medium ${pizza.stock === true ? "text-emerald-800 border-green-800" : "text-red-600 border-red-600"}`}>{pizza.stock === true ? "In Stock" : "Out Of Stock"}</span>
                  </div>
                  <p className="text-muted-foreground text-sm mb-4">
                    {pizza.description}
                  </p>
                  <div className="flex items-center justify-between">
                    <span className="text-muted-foreground text-sm">{pizza.category}</span>
                  ...
                  {pizza.stock === true ? 
                  <button className={`mt-2 w-full inline-flex items-center justify-center whitespace-nowrap text-sm font-medium focus-visible:outline-none text-orange-600 hover:bg-orange-200 hover:text-black duration-300 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 border  h-9 rounded-md px-3 `}>
                  Order Now
                </button> : <button className={`mt-2 w-full inline-flex items-center justify-center whitespace-nowrap text-sm font-medium disabled text-orange-600 hover:bg-orange-200 hover:text-black duration-300  border  h-9 rounded-md px-3 `}>
                  Order Now
                </button>}
